...
public class AStar {
        ...
        for (int i = currentY - 1; i <= currentY + 1; i++) {
            ...
            for (int j = currentX - 1; j <= currentX + 1; j++) {
                ...
                if (!(currentX == j && currentY == i)) {
                    addNeighbourNode(current, grid[i][j],
                            isDiagonal(current, grid[i][j]) ? DIAG_COST : DEF_COST);
                }
            }
    ...
    /**
     * Sets the status of all nodes on the path to reflect that the path goes through them And
     * prints the grid
     */
    ...
    /**
     * Updates all status on found nodes, shows their cost if they haven't been closed yet,
     * otherwise shows CL And prints the grid
     */
